...
def breaking_news(limit=12):
    ...
    for n in data.get('news',[]):
        ...
        dt=parse_api_date(n)
        # Breaking News sollen aktuell sein; alte Meldungen verschwinden nach spätestens 30 Stunden.
        if dt and now-dt>timedelta(hours=30):continue
        candidates.append((dt or now,country_prefix(n)+' · '+t))
    candidates.sort(key=lambda x:x[0],reverse=True)
    return dedupe([t for _,t in candidates],limit)

# Google News auf die letzten 24 Stunden begrenzen; maximal 30 Stunden Toleranz für Zeitstempel.
q=urllib.parse.quote('(Dorsten OR Wulfen OR Lembeck OR Rhade OR Hervest OR "Holsterhausen Dorsten") when:1d')
...
# Nur bei einem echten Abruffehler den letzten Stand behalten.
